...
describe('preferences persistence', () => {
  it('merges push map + quietHours into user.preferences.notifications', async () => {
    const user = { id: 'u1', preferences: { theme: 'dark', notifications: { push: { 'message:whatsapp': true } } } };
    const userRepo = {
    ...
    expect(res.push['message:whatsapp']).toBe(true);
    ...
    expect(userRepo.save.mock.calls[0][0].preferences.notifications.push['message:whatsapp']).toBe(true);
